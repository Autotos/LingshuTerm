...
**仅对代码修改类任务**（安装依赖、构建、git 操作）自动执行验收。纯查询任务（列出文件、统计数量、查看状态）跳过验收。
...
1. 根据上下文中的"当前操作系统"选择正确命令语法
2. 不确定环境时首先生成 `uname -s`（Unix）或 `ver`（Windows）
3. 按平台选择包管理器：Windows=winget, macOS=brew, Debian=apt, RHEL=dnf, Arch=pacman
